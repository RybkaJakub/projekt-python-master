...
datum = "12. 10. 2020"
d, m, r = datum.split(". ")
databazovy_format = f"{r}-{m.zfill(2)}-{d.zfill(2)}"
print(databazovy_format)

def generate_identifiers(souslovi):
    python_identifier = souslovi.lower().replace(' ', '_')
    js_identifier = ''.join(word.capitalize() for word in souslovi.split())

    return python_identifier, js_identifier


souslovi = "To je proměnná v Pythonu"
python_identifier, js_identifier = generate_identifiers(souslovi)

print(f"Python identifier: {python_identifier}")
print(f"JS camelCase identifier: {js_identifier}")
...
def generate_random_password():
    ...
    password = ''.join(uppercase + lowercase + [special_char] + digits)
    return password


pocet_osob = 5
hesla = [generate_random_password() for _ in range(pocet_osob)]
